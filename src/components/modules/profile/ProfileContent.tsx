...
export function ProfileContent({ user }: { user?: User }) {
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-6 mt-6">
      ...
      {/* Sol Panel: Sidebar Widgets */}
      <div className="w-full lg:w-80 shrink-0 space-y-6">
        <ProfileBadgesWidget />

        {isOwnProfile && (
          <ProfileCloudWidget onManageCloud={() => setIsCloudModalOpen(true)} />
        )}
        ...
        <ProfileFriendsWidget
          ...
          onSeeAll={() => setActiveTab('Arkadaşlar')}
        />
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-6">
        ...
        <div className="min-h-[400px]">
          ...
          {activeTab === 'Kariyer' && <CareerTab displayUser={displayUser || null} />}

          {activeTab === 'Oynadığı Oyunlar' && <GamesTab user={displayUser || null} />}
          ...
          {activeTab === 'Arkadaşlar' && (
            <FriendsTab 
              ...
              totalCount={displayUser?.friendCount} 
              isLoading={isLoadingFriends}
              hasMore={hasMoreFriends}
              onLoadMore={() => fetchFriends(true)}
            />
          )}
        </div>
