      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-armoyu-text-muted group-focus-within:text-armoyu-primary transition-colors" size={16} />
            <input
              type="text"
              placeholder="Arkadaşlarda ara..."
              className="bg-armoyu-card-bg border border-armoyu-card-border rounded-2xl pl-11 pr-4 py-3 text-xs font-bold text-armoyu-text focus:outline-none focus:border-armoyu-primary transition-all w-full md:w-64"
            />
          </div>
          <button className="p-3 bg-armoyu-primary text-white rounded-2xl hover:bg-armoyu-primary transition-all shadow-lg shadow-armoyu-primary/20">
            <UserPlus size={20} />
          </button>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {friends.map((friend) => (
              <div
                key={friend.id}
                className="bg-armoyu-card-bg border border-armoyu-card-border rounded-[32px] p-6 shadow-sm hover:shadow-xl hover:shadow-armoyu-primary/5 transition-all group relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-24 h-24 bg-armoyu-primary/5 blur-2xl rounded-full" />

                <div className="flex flex-col items-center text-center relative z-10">
                  <div className="relative mb-4">
                    <div className="w-20 h-20 rounded-[28px] overflow-hidden border-2 border-armoyu-card-border group-hover:border-armoyu-primary transition-colors bg-black/5">
                  ...
                  <p className="text-xs font-bold text-armoyu-primary mb-4 lowercase">@{friend.username}</p>

                  <button className="w-full py-2.5 rounded-xl border border-armoyu-card-border text-[10px] font-black text-armoyu-text-muted hover:text-armoyu-primary hover:border-armoyu-primary/50 hover:bg-armoyu-primary/5 transition-all uppercase tracking-widest">
                    Profili Gör
                  </button>
                </div>
              </div>
          ...
          {hasMore && onLoadMore && (
            <div className="flex justify-center mt-6">
              <button
                onClick={onLoadMore}
                disabled={loading}
                className="px-6 py-3 bg-armoyu-primary hover:bg-armoyu-primary disabled:bg-armoyu-primary/50 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-armoyu-primary/20 active:scale-95 flex items-center gap-2"
              >
                {loading ? (
