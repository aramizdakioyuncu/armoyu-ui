  ...
  return (
    <div className="space-y-8">
      <div className="bg-armoyu-card-bg border border-armoyu-card-border rounded-[40px] p-10 shadow-sm relative overflow-hidden">
        <div className="absolute -right-20 -top-20 w-64 h-64 bg-armoyu-primary/5 blur-3xl rounded-full" />

        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 relative z-10">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-3xl bg-armoyu-primary flex items-center justify-center text-white shadow-xl shadow-armoyu-primary/20">
              <Milestone size={32} />
            </div>
            <div>
        ...
        <div className="relative space-y-12 before:absolute before:inset-0 before:ml-8 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-armoyu-primary/50 before:via-armoyu-primary/20 before:to-transparent">
          {timeline.map((event, idx) => (
            <div key={event.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              {/* Icon */}
              <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-armoyu-card-bg border-4 border-armoyu-card-border shadow-md z-10 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 transition-transform group-hover:scale-110 group-hover:border-armoyu-primary">
                {event.type === 'JOIN' && <Award className="text-armoyu-primary" size={24} />}
              ...
              {/* Content */}
              <div className="w-[calc(100%-5rem)] md:w-[calc(50%-4rem)] p-6 rounded-[32px] bg-black/5 dark:bg-white/5 border border-transparent hover:border-armoyu-primary/20 transition-all group-hover:bg-armoyu-primary/5">
                <div className="flex items-center justify-between mb-2">
                  <time className="text-[10px] font-black text-armoyu-primary uppercase tracking-widest bg-armoyu-primary/10 px-3 py-1 rounded-full">{event.date}</time>
                </div>
