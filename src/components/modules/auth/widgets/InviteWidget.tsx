    ...
    return (
        <div className="flex flex-col h-full animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 hide-scrollbar">
                {/* Invite Code Card */}
                <div className="bg-gradient-to-br from-armoyu-primary to-indigo-600 rounded-[32px] p-6 text-white shadow-xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-4">
                        ...
                        <div className="flex items-center justify-between bg-black/40 rounded-2xl p-4 border border-white/10">
                            <span className="text-2xl font-black tracking-widest font-mono">{inviteCode || '------'}</span>
                            <button
                                onClick={copyToClipboard}
                                className="flex items-center gap-2 px-3 py-2 bg-white text-armoyu-primary rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-armoyu-primary/5 transition-all active:scale-95"
                            >
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="bg-armoyu-card-bg border border-armoyu-drawer-border rounded-3xl p-4 flex flex-col items-center justify-center text-center gap-1 shadow-sm">
                        <span className="text-xl font-black text-armoyu-primary italic">0</span>
                    ...
                    <div className="space-y-2">
                        {isLoading ? (
                            Array.from({ length: 3 }).map((_, i) => (
                                <div key={i} className="flex items-center gap-3 p-3 bg-white/10 rounded-2xl border border-white/10 animate-pulse">
                            ...
                            invitations.map((inv, idx) => (
                                <div key={idx} className="flex items-center gap-3 p-3 bg-white/5 rounded-2xl border border-white/5 hover:border-armoyu-primary/30 transition-all group/item">
                                    <img
                                        src={inv.avatar}
                                    ...
                                    {inv.verified ? (
                                        ...
                                        <button
                                            ...
                                            className="p-2 bg-armoyu-primary/10 hover:bg-armoyu-primary text-armoyu-primary hover:text-white rounded-lg transition-all"
            ...
            <div className="p-6 mt-auto">
                <button
                    onClick={copyToClipboard}
                    className="w-full py-4 bg-white/5 hover:bg-armoyu-primary hover:text-white rounded-2xl border border-white/10 text-armoyu-text font-black text-[10px] uppercase tracking-widest transition-all shadow-xl flex items-center justify-center gap-3 group"
                >
