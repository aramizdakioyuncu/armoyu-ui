...
export function NewsDetailAuthor({ name, avatar, bio }: NewsDetailAuthorProps) {
  return (
    <div className="p-8 bg-gradient-to-br from-blue-600/10 to-purple-600/10 border border-white/5 rounded-[40px] flex items-center gap-6">
      <div className="w-20 h-20 rounded-3xl overflow-hidden border-2 border-blue-600/20 flex-shrink-0">
        {avatar ? (
          <img src={avatar} alt={name} className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center bg-zinc-800 text-blue-500">
            <User size={32} />
          </div>
        )}
      </div>
      <div>
        <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest leading-none mb-1 inline-block">Yazar</span>
