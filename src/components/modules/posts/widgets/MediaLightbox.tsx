...
export function MediaLightbox({ isOpen, onClose, media, initialIndex = 0, defaultOwner, showComments = true }: MediaLightboxProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[10001] flex items-center justify-center p-0 md:p-10 animate-in fade-in duration-300">
      ...
      {/* Container */}
      <div 
        className={`relative w-full h-full bg-white dark:bg-[#0a0a0f] rounded-none md:rounded-[40px] overflow-hidden flex flex-col md:flex-row border-none md:border border-slate-200 dark:border-armoyu-primary/20 shadow-[0_0_100px_rgba(0,0,0,0.1)] dark:shadow-[0_0_80px_rgba(var(--armoyu-primary-rgb),0.15)] z-[10000] ${
          showComments ? 'max-w-[1400px]' : 'max-w-[1000px]'
        }`}
      ...
      >
        ...
        <div className="flex-1 relative flex items-center justify-center bg-slate-100 dark:bg-black/20 group">
          ...
          <button 
            onClick={onClose} 
            className={`absolute top-6 right-6 w-10 h-10 rounded-full bg-black/5 dark:bg-white/10 text-slate-800 dark:text-white flex items-center justify-center z-30 ${
              showComments ? 'md:hidden' : 'block'
            }`}
          >
