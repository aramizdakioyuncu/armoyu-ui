...
export function SchoolDetailPage() {
  ...
  const mockTeams = [
    ...
    { id: 3, name: 'League of Legends', game: 'LoL', members: 7, logo: 'https://storage.aramizdakioyuncu.com/galeri/oyunresimleri/lol.png', color: 'bg-blue-500' },
  ...
  if (!school) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-6 bg-armoyu-bg">
        ...
        <button onClick={() => router.push('/?tab=egitim')} className="px-8 py-3 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest">Listeye Dön</button>
      </div>
    );
  }
  ...
  return (
    <div className="pb-32 bg-armoyu-bg min-h-screen">
      ...
      <div className="relative h-[400px] w-full overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-blue-600/20 to-armoyu-bg z-10" />
        <img 
          src={school.logo} 
            ...
            <div className="flex flex-col md:flex-row items-center md:items-end gap-8">
              ...
              <div className="flex-1 text-center md:text-left mb-4">
                <div className="flex flex-wrap justify-center md:justify-start items-center gap-3 mb-4">
                  <span className="px-4 py-1 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-full italic shadow-lg shadow-blue-600/20">Onaylı Kampüs</span>
              ...
              <div className="flex gap-4">
                 <button className="px-8 py-4 bg-blue-600 text-white rounded-[24px] font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-600/20 hover:scale-105 active:scale-95 transition-all">OKULUNA KATIL</button>
              </div>
            </div>
          </div>
        </div>
            ...
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2.5 px-2 py-2 whitespace-nowrap text-[10px] font-black uppercase tracking-[0.2em] transition-all relative group ${
                  activeTab === tab.id ? 'text-blue-500' : 'text-armoyu-text-muted hover:text-armoyu-text'
                }`}
              >
                {tab.icon}
                {tab.label}
                {activeTab === tab.id && (
                  <span className="absolute -bottom-4 left-0 w-full h-1 bg-blue-600 rounded-full animate-in slide-in-from-left duration-300" />
                )}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-[1280px] mx-auto px-6 md:px-12 pt-12">
        ...
        {activeTab === 'genel' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 animate-in fade-in duration-500">
            <div className="lg:col-span-2 space-y-12">
              <div className="glass-panel p-10 rounded-[40px] border border-armoyu-card-border">
                <h3 className="text-xl font-black text-armoyu-text mb-6 uppercase italic tracking-widest flex items-center gap-3">
                  <Info className="text-blue-500" /> OKUL HAKKINDA
                </h3>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {[
                  { label: 'ÖĞRENCİ SAYISI', value: school.memberCount, icon: <Users />, color: 'text-blue-500' },
                  ...
                  <div key={i} className="glass-panel p-6 rounded-[30px] border border-armoyu-card-border text-center group hover:border-blue-500/30 transition-colors">
            ...
            <div className="space-y-8">
              <div className="glass-panel p-8 rounded-[40px] border border-armoyu-card-border bg-blue-600/5">
                <h3 className="text-sm font-black text-armoyu-text mb-6 uppercase italic tracking-widest">KAMPÜS TEMSİLCİSİ</h3>
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-blue-600 border-2 border-white/10 overflow-hidden shadow-xl shadow-blue-600/20">
                  ...
                  <div>
                    ...
                    <div className="text-[9px] font-bold text-blue-500 uppercase tracking-widest mt-1 italic">RESMİ TEMSİLCİ</div>
                  </div>
                </div>
        ...
        {activeTab === 'takimlar' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 animate-in slide-in-from-bottom-8 duration-500">
            {mockTeams.map((team) => (
              <div key={team.id} className="glass-panel p-8 rounded-[40px] border border-armoyu-card-border group hover:border-blue-500/50 hover:shadow-2xl transition-all duration-500 overflow-hidden relative">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-xl font-black text-armoyu-text mb-2 uppercase italic tracking-tighter leading-tight group-hover:text-blue-500 transition-colors">
                    {team.name}
                  </h3>
                  <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/5">
                    ...
                    <button className="p-2 rounded-xl bg-white/5 text-armoyu-text-muted hover:text-white hover:bg-blue-600 transition-all">
            ...
            {/* Create Team Card */}
            <div className="glass-panel p-8 rounded-[40px] border border-dashed border-white/10 flex flex-col items-center justify-center text-center group hover:border-blue-500/50 transition-all cursor-pointer bg-white/5">
                <div className="w-16 h-16 rounded-full bg-blue-600/10 flex items-center justify-center text-blue-500 mb-6 group-hover:scale-110 transition-transform">
                  <Plus size={32} />
                </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
             <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
                ...
                <div className="w-full md:w-[300px] relative">
                   <input type="text" placeholder="SINIF ARA..." className="w-full h-12 pl-12 pr-4 bg-white/5 border border-white/5 rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none focus:border-blue-500 transition-all" />
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {classrooms.length > 0 ? classrooms.map((cls) => (
                  <div key={cls.id} className="glass-panel p-6 rounded-[30px] border border-armoyu-card-border text-center group hover:bg-blue-600 hover:border-blue-500 transition-all cursor-pointer">
