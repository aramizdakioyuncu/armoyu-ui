'use client';

import React, { useState, useEffect } from 'react';
import { useAuth } from '../../../../context/AuthContext';
import { useArmoyu } from '../../../../context/ArmoyuContext';
import { AnimatedStat } from '../../../shared/AnimatedStat';

export function EconomyWidget() {
  const { user: currentUser } = useAuth();
  const { api } = useArmoyu();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<any>(null);

  useEffect(() => {
    async function fetchEconomy() {
      if (!currentUser) {
        setData(null);
        setLoading(false);
        return;
      }
      setLoading(true);
      try {
        const response = await api.siteInfo.getMarketCurrencies();
        if (response.durum === 1 && response.icerik) {
          setData(response.icerik);
        }
      } catch (error) {
        console.error('Failed to fetch economy stats:', error);
      } finally {
        setLoading(false);
      }
    }
    fetchEconomy();
  }, [api, currentUser]);

  // Map API data to UI structure
  const currencies = (data && data.length > 0) ? data.map((c: any) => ({
    code: c.code,
    name: c.code === 'XAU' ? 'Gram Altın' : c.code === 'USD' ? 'Dolar' : c.code === 'EUR' ? 'Euro' : c.code === 'GBP' ? 'Sterlin' : c.code,
    source: 'Merkez Bankası',
    price: c.value,
    logo: c.logo,
    trend: 'up', // API doesn't provide trend yet
    change: '+0.00%',
    color: c.code === 'XAU' ? 'armoyu-warning' : c.code === 'USD' ? 'armoyu-success' : 'blue'
  })) : [
    { code: 'USD', name: 'Dolar', source: 'Merkez Bankası', price: '32.45', trend: 'up', change: '+0.24%', color: 'armoyu-success' },
    { code: 'EUR', name: 'Euro', source: 'Merkez Bankası', price: '35.12', trend: 'up', change: '+0.15%', color: 'blue' },
    { code: 'XAU', name: 'Gram Altın', source: 'Spot Piyasa', price: '2,450', trend: 'down', change: '-0.12%', color: 'armoyu-warning' },
    { code: 'GBP', name: 'Sterlin', source: 'Merkez Bankası', price: '41.20', trend: 'up', change: '+0.45%', color: 'slate' }
  ];

  return (
    <div className="glass-panel p-5 rounded-3xl border border-armoyu-card-border bg-armoyu-card-bg">
      <div className="flex items-center gap-2 mb-4">
         <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-armoyu-primary"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>
         <h3 className="font-extrabold text-armoyu-text text-base">Ekonomi</h3>
      </div>
      
      <div className="space-y-4">
         {currencies.map((c: any, i: number) => (
           <div key={i} className={`flex items-center justify-between p-3 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 ${loading ? 'animate-pulse' : ''}`}>
             <div className="flex items-center gap-2.5">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs overflow-hidden ${
                  c.color === 'armoyu-success' ? 'bg-armoyu-primary/10 text-armoyu-primary' : 
                  c.color === 'armoyu-warning' ? 'bg-armoyu-warning/10 text-armoyu-warning' : 
                  'bg-slate-400/20 text-slate-500 dark:text-slate-300'
                }`}>
                  {c.logo ? (
                    <img src={c.logo} className="w-full h-full object-cover" alt={c.code} />
                  ) : (
                    c.code
                  )}
                </div>
                <div>
                   <span className="block text-xs font-bold text-armoyu-text">{c.name}</span>
                   <span className="block text-[10px] text-armoyu-text-muted">{c.source}</span>
                </div>
             </div>
             <div className="text-right">
                <div className="block text-sm font-black text-armoyu-text">
                  <AnimatedStat value={c.price} prefix="₺" />
                </div>
                <span className={`flex items-center justify-end text-[10px] font-bold ${c.trend === 'up' ? 'text-armoyu-success' : 'text-armoyu-danger'}`}>
                   <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className={`mr-0.5 ${c.trend === 'down' ? 'rotate-180' : ''}`}><polyline points="18 15 12 9 6 15"></polyline></svg>
                   {c.change}
                </span>
             </div>
           </div>
         ))}
      </div>
    </div>
  );
}
