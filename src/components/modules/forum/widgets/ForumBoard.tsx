...
export function ForumBoard({ id, name, desc, topicCount, postCount, lastPost, icon }: ForumBoardProps) {
  return (
    <Link href={`/forum/${id}`} className="block">
      <div className="group flex flex-col md:flex-row items-center gap-4 p-4 md:p-6 glass-panel bg-armoyu-card-bg border border-armoyu-card-border hover:shadow-2xl transition-all duration-300 hover:border-armoyu-primary/30">
        
        {/* Icon */}
        <div className="w-16 h-16 rounded-[22px] bg-armoyu-primary/10 dark:bg-white/5 flex items-center justify-center text-armoyu-primary group-hover:bg-armoyu-primary group-hover:text-white transition-all shrink-0">
        ...
        <div className="flex-1 min-w-0 text-center md:text-left">
           <h3 className="text-xl font-black text-armoyu-text mb-1 uppercase tracking-tight group-hover:text-armoyu-primary transition-colors">
              {name}
           </h3>
        ...
        <div className="hidden xl:flex items-center gap-4 w-64 shrink-0">
            {lastPost ? (
              <>
                 <img src={lastPost.avatar || undefined} alt={lastPost.author} className="w-10 h-10 rounded-full border border-armoyu-primary/20" />
                 <div className="min-w-0">
                    <p className="text-xs font-bold text-armoyu-text truncate hover:text-armoyu-primary cursor-pointer">{lastPost.topicTitle}</p>
                    <p className="text-[10px] font-medium text-armoyu-text-muted mt-0.5 uppercase tracking-tighter">
                       <span className="text-armoyu-primary font-bold">{lastPost.author}</span> • {lastPost.time}
                    </p>
                 </div>
              </>
            ) : (
