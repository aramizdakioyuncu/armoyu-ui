   ...
   return (
      <div className="glass-panel p-8 rounded-[40px] border border-armoyu-card-border bg-armoyu-card-bg relative overflow-hidden group/card">
         <div className="flex items-center justify-between mb-8 leading-none relative z-10">
            ...
            <Link href={`${groupUrl}/uyeler`} className="text-[10px] font-black text-blue-500 uppercase flex items-center gap-1 hover:gap-2 transition-all leading-none group">
                  ...
                  <Link key={idx} href={`/oyuncular/${member.username}`} className="flex items-center gap-4 group cursor-pointer transition-all hover:translate-x-1 duration-300">
                     <div className="relative">
                        <img 
                           ...
                           className="w-12 h-12 rounded-2xl border-2 border-transparent group-hover:border-blue-500/50 transition-all object-cover shadow-lg" 
                           alt="Avatar" 
                        />
                        {idx < 3 && (
                     ...
                     <div className="flex-1 min-w-0">
                        <p className="text-sm font-black text-armoyu-text mb-1 group-hover:text-blue-500 transition-colors uppercase italic leading-none truncate">
                           {member.displayName}
                        </p>
                        <p className={`text-[9px] font-black uppercase tracking-widest leading-none ${idx === 0 ? 'text-blue-500' : 'text-armoyu-text-muted opacity-60'}`}>
