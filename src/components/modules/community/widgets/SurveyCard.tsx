...
export function SurveyCard({ survey, onVote }: SurveyCardProps) {
  ...
  return (
    <div className="glass-panel relative overflow-hidden rounded-[40px] border border-armoyu-card-border bg-armoyu-card-bg p-8 shadow-sm hover:shadow-2xl transition-all duration-500 group">
      ...
      <div className="absolute top-0 right-0 w-64 h-64 bg-armoyu-primary/5 blur-[100px] -mr-32 -mt-32 pointer-events-none" />
      ...
      <div className="flex items-start justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-armoyu-primary/10 flex items-center justify-center text-armoyu-primary border border-armoyu-primary/20 group-hover:scale-110 transition-transform">
            <Trophy size={24} />
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className={`px-2 py-0.5 text-white text-[8px] font-black uppercase tracking-widest rounded-md ${isExpired ? 'bg-red-600' : 'bg-armoyu-primary'}`}>
      ...
      <div className="mb-10 relative z-10">
        <h2 className="text-2xl md:text-3xl font-black text-armoyu-text uppercase tracking-tighter italic leading-tight mb-4 group-hover:text-armoyu-primary transition-colors">
          {survey.question}
        </h2>
          ...
          return (
            <button
              ...
              className={`w-full relative group/option rounded-2xl border transition-all duration-300 overflow-hidden ${
                hasVoted 
                  ? isSelected ? 'border-armoyu-primary bg-armoyu-primary/5' : 'border-armoyu-card-border bg-black/5 dark:bg-white/5 opacity-80'
                  : isExpired 
                    ? 'border-armoyu-card-border bg-black/5 dark:bg-white/5 opacity-50 cursor-not-allowed'
                    : 'border-armoyu-card-border bg-black/5 dark:bg-white/5 hover:border-armoyu-primary/50 hover:bg-black/10'
              }`}
            >
              {/* Progress Bar (Visible after voting) */}
              {hasVoted && (
                <div 
                  className={`absolute inset-y-0 left-0 transition-all duration-1000 ease-out ${isSelected ? 'bg-armoyu-primary/20' : 'bg-black/10 dark:bg-white/5'}`}
              ...
              <div className="relative p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`transition-colors ${isSelected ? 'text-armoyu-primary' : 'text-armoyu-text-muted'}`}>
                    {hasVoted ? (
                      ...
                      <Circle size={20} className="group-hover/option:text-armoyu-primary transition-colors" />
                    )}
                  </div>
                  <span className={`text-sm font-black uppercase tracking-widest transition-colors ${isSelected ? 'text-armoyu-primary' : 'text-armoyu-text'}`}>
                    {option.text}
                  </span>
                </div>
                ...
                {hasVoted && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-lg font-black italic tracking-tighter ${isSelected ? 'text-armoyu-primary' : 'text-armoyu-text-muted'}`}>
                      %{percentage}
                    </span>
                  </div>
                )}
      ...
      <div className="mt-10 pt-8 border-t border-armoyu-card-border flex items-center justify-between relative z-10">
        <div className="flex items-center gap-2 text-armoyu-text-muted">
           <Timer size={14} className={isExpired ? "text-red-500" : "text-armoyu-primary"} />
           <span className="text-[9px] font-black uppercase tracking-[0.2em]">
             {isExpired ? 'ANKET SONUÇLANDI' : `BİTİŞ: ${survey.expiresAt}`}
           </span>
        </div>
        {!hasVoted && !isExpired && (
           <div className="flex items-center gap-2 text-armoyu-primary text-[10px] font-black uppercase tracking-widest animate-pulse">
