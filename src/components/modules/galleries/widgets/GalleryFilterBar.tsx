      ...
      <div className="flex flex-col md:flex-row gap-6 items-center justify-between mb-12">
         <div className="flex bg-black/5 dark:bg-white/5 p-1.5 rounded-[22px] border border-armoyu-card-border overflow-x-auto no-scrollbar max-w-full">
            {categories.map((cat) => (
               <button
                  ...
                  className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${activeTab === cat ? 'bg-armoyu-primary text-white shadow-xl' : 'text-armoyu-text-muted hover:text-armoyu-text hover:bg-black/5 dark:hover:bg-white/5'}`}
               >
                  {cat}
               </button>
            ))}
         ...
         <div className="relative w-full md:w-80">
            <input
               ...
               className="w-full bg-black/5 dark:bg-white/5 border border-armoyu-card-border rounded-2xl px-6 py-3.5 text-sm text-armoyu-text focus:outline-none focus:border-armoyu-primary focus:ring-1 focus:ring-armoyu-primary transition-all font-bold placeholder:text-armoyu-text-muted/40"
            />
