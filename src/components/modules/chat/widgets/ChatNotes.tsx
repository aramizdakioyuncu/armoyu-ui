  ...
  return (
    <div className="flex flex-col items-center shrink-0 cursor-pointer group relative">
      <div className="relative mb-3 flex flex-col items-center">
        ...
        {/* Thought Bubble Box */}
        <div className={`relative px-3 py-1.5 rounded-2xl text-[10px] font-bold max-w-[85px] transition-all group-hover:-translate-y-1 shadow-sm border border-black/5 dark:border-white/10 ${isMe
        ...
        <div className="mt-1 relative">
          <img
            src={note.avatar}
            alt={note.username}
            className="w-11 h-11 md:w-12 md:h-12 rounded-full border-2 border-white dark:border-white/10 shadow-sm object-cover"
          />
          {isMe && (
      ...
      <span className={`text-[10px] text-armoyu-text-muted font-medium truncate w-14 text-center ${isMe ? 'italic' : ''}`}>
        {isMe ? 'Hikayen' : note.username}
      </span>
    </div>
  );
}
