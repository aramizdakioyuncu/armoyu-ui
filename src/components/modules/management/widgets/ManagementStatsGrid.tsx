...
export const ManagementStatsGrid = ({ stats }: { stats: StatItem[] }) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {stats.map((stat) => (
        <div key={stat.name} className="bg-white dark:bg-white/5 border border-black/5 dark:border-white/5 rounded-[32px] p-6 hover:border-blue-500/30 transition-all group overflow-hidden relative shadow-sm">
          ...
          <div className="text-3xl font-black text-armoyu-text mb-1 italic">{stat.value}</div>
