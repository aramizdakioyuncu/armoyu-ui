...
export const MentionText: React.FC<MentionTextProps> = ({ text, className }) => {
  // Regex to find @username (alphanumeric and underscore)
  ...
  return (
    <div className={className}>
      {parts.map((part, i) => {
        // If the part is followed by a mention, render the part then the mention
        const currentMention = mentions && i < parts.length - 1 ? mentions[mentionIndex++] : null;
        
        return (
          <React.Fragment key={i}>
            {part}
            {currentMention && (
              <MentionLink username={currentMention.substring(1)} />
            )}
          </React.Fragment>
        );
      })}
        ...
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-64 bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl p-4 animate-in fade-in zoom-in-95 duration-200 z-50 pointer-events-none">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="min-w-0">
              <div className="text-sm font-black text-armoyu-text truncate uppercase tracking-tight italic">@{username}</div>
              <div className="text-[10px] font-bold text-armoyu-text-muted uppercase tracking-widest">Oyuncu Profili</div>
            </div>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-black/5 dark:border-white/5 flex items-center justify-between">
            <span className="text-[10px] font-bold text-armoyu-text-muted">Profilini görüntülemek için tıkla</span>
