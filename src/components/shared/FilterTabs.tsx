      ...
      <div className={`flex border-b border-armoyu-card-border overflow-x-auto no-scrollbar ${className}`}>
        {tabs.map((tab) => (
          ...
          >
            <span className="flex items-center gap-2">
              ...
              {counts?.[tab] !== undefined && (
                <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${
                  active === tab
                    ? 'bg-armoyu-primary/10 text-armoyu-primary'
            ...
            {active === tab && (
              <div className="absolute bottom-0 left-3 right-3 h-0.5 bg-armoyu-primary rounded-full" />
            )}
          </button>
        ))}
      </div>
      ...
      {tabs.map((tab) => (
        <button
          ...
          className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shrink-0 ${
            active === tab
              ? 'bg-white dark:bg-armoyu-primary text-armoyu-primary dark:text-white shadow-xl'
        ...
        >
          <span className="flex items-center gap-2">
            ...
            {counts?.[tab] !== undefined && (
              <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${
                active === tab
                  ? 'bg-armoyu-primary/20 dark:bg-white/20'
